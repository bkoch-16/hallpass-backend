...
**Resolution:** replaced order-dependent `Once` chains with an args-keyed `mockImplementation`: `authenticateAs()` answers requireAuth's lookup (matched by its unique shape — no `select`, `where.id` = session user, no `schoolId` scope), and route-handler target lookups go through a per-test `givenTargetUser()` / `givenTargetUserLookupError()` override that defaults to "not found". Self-targeted existence checks share the requireAuth shape and correctly return the session user. `findFirst` call count/order no longer affects any assertion.

**Reopened by 2026-07-07 CR:** the suite still flaked intermittently after the findFirst fix (~1/15 full-suite, ~1/30 single-file runs; signatures: 403-vs-200 flip in a converted test, 'socket hang up' on the bulk 400 test, a 5s timeout on an unauthenticated DELETE, and cascading bulk-create failures). Two causes: (a) `beforeEach` used `vi.clearAllMocks()`, which clears call history but not queued `mockResolvedValueOnce` implementations on `user.create`, so an aborted request leaked unconsumed Once values into the next test — fixed by switching to `vi.resetAllMocks()`. (b) An underlying transport trigger aborted requests mid-flight, both failing tests directly and feeding (a).

**Transport root cause (proven with a live capture):** `request(app)` makes supertest create a wildcard-bound server (`listen(0)` → `*:PORT`) per request, then connect to `127.0.0.1:PORT`. On macOS, a foreign process can bind `127.0.0.1:PORT` while the wildcard listener is live (BSD specific-over-wildcard bind semantics), and the kernel routes the connection to the more-specific foreign listener — captured live with JetBrains Toolbox mid-self-update holding `127.0.0.1:62913` against the test's `*:62913`. Foreign listeners that hang produce the 5s vitest timeout, ones that reset produce 'socket hang up', and ones that speak HTTP produce the status flips (a JetBrains port answers 403, IDEA ports 404 — the original 403→404 signature).

**Resolution:** each supertest file (`tests/routes/user.test.ts`, `tests/app.test.ts`, `tests/integration/user.test.ts`) now creates one shared `http.Server` bound explicitly to `127.0.0.1` in `beforeAll` (an exact-match bind the kernel protects — steal attempts fail with `EADDRINUSE`) and passes it to `request(server)`. Validated during investigation: 2000/2000 requests survived 20 planted poison listeners; 30/30 suite runs green. Relatedly, `createGeneralLimiter` now defaults to an effectively unlimited limit under `NODE_ENV === "test"` (see item 13) — the shared in-memory IP-keyed counter was 77/100 in `routes/user.test.ts` and ~23 more tests would have started deterministic 429 cascades.
