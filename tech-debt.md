...
**Resolution:** each supertest file (`tests/routes/user.test.ts`, `tests/app.test.ts`, `tests/integration/user.test.ts`) now creates one shared `http.Server` bound explicitly to `127.0.0.1` in `beforeAll` (an exact-match bind the kernel protects — steal attempts fail with `EADDRINUSE`) and passes it to `request(server)`. Validated during investigation: 2000/2000 requests survived 20 planted poison listeners; 30/30 suite runs green. Relatedly, `app.ts` now raises the general limiter's limit under `NODE_ENV === "test"` — the shared in-memory IP-keyed counter was 77/100 in `routes/user.test.ts` and ~23 more tests would have started deterministic 429 cascades.
