...
### 12. Mock-ordering fragility in user-api route tests — PARTIALLY RESOLVED 2026-07-07
...
**Remaining (reopened by 2026-07-07 CR):** the suite still flaked intermittently after the findFirst fix (~1/15 full-suite, ~1/30 single-file runs; signatures: 403-vs-200 flip in a converted test, 'socket hang up' on the bulk 400 test, a 5s timeout on an unauthenticated DELETE, and cascading bulk-create failures). Two causes: (a) `beforeEach` used `vi.clearAllMocks()`, which clears call history but not queued `mockResolvedValueOnce` implementations on `user.create`, so an aborted request leaked unconsumed Once values into the next test — fixed by switching to `vi.resetAllMocks()`. (b) The underlying transport trigger ('socket hang up' / 5s timeout under supertest + parallel vitest workers) is still unexplained and can fail a test on its own; needs separate investigation before this item can be closed.
